        ...
        for(int y=0; y<height; y += meshSimpleIncrement) {
            for(int x=0; x<width; x += meshSimpleIncrement) {
                ...
                if(x < width - 1 && y < height - 1) {
                    int vw  = vertexIndex  + verticesPerLine;
                    int vwo = vw + 1;
                    int vo  = vertexIndex + 1;
                    meshData.AddTriangle(vertexIndex, vwo, vw);
                    meshData.AddTriangle(vwo, vertexIndex, vo);
                }
                vertexIndex++;
            }
        }
